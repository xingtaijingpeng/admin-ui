<template>
	...
	>
		<a-divider orientation="left">评论添加/修改</a-divider>
		...
		<a-form-item label="描述">
			<a-input
					v-decorator="['contents',{initialValue: '',rules: [{ required: true, message: 'Please input your route_name!' }]}]"
					placeholder="请输入描述"
			/>
		</a-form-item>
    ...
    export default {
		...
		mounted(){
            ...
            if(_this.$route.params.sid){
                axios.post('comments/detail/'+_this.$route.params.sid,{}).then((response) => {
                    if(!response.status){
        ...
        methods: {
            ...
            handleSubmit(e) {
                ...
                _this.form.validateFields((err, values) => {
                    ...
                    let url;
                    if(_this.$route.params.sid){
                        url = 'comments/update/'+_this.$route.params.sid;
                    }else{
                        url = 'comments/create';
                    }

                    axios.post(url,{...values,aid: _this.$route.params.id}).then((response) => {
