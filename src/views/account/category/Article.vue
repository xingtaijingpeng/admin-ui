<template>
	<s-table ref="table" :columns="columns" dataUrl="category/index?guard=article&merge=1" :pagination="false">
		<template slot="title">
			<a-row type="flex" justify="space-around" align="middle">
				<a-col :span="8">文章分类</a-col>
				<a-col :span="16" :style="{ textAlign:'right'}">
					<a-button type="primary" icon="plus" @click="jump('/account/category/article/create/article')">添加分类</a-button>
				</a-col>
			</a-row>
		</template>
		...
		<template slot="operation" slot-scope="data, record">
			<a @click="jump('/account/category/article/update/article/'+record.id)">编辑</a>
